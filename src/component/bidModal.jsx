...
const BidModal = ({ isModalOpen, onClose, title, price, productId, refetch, refetchBids }) => {
  ...
  const handleOnBid = async () => {
    try {
      if (Number(bidPrice) <= Number(price)) {
        message.error("Bid must be higher than the minimum price");
        return;
      }
  ...
  return (
    ...
    >
      ...
      <Button
        onClick={handleOnBid}
        loading={loading}
        disabled={Number(bidPrice) <= Number(price)}
